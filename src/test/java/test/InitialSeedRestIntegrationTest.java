...
public class InitialSeedRestIntegrationTest {
    ...
    @Test
    public void testRestForUsersById() {
        login("Anne", "test");
        given()
                .contentType("application/json")
                .header("Authorization", "Bearer " + securityToken)
                .when()
                .get("/api/user/Anne").then()
                .statusCode(200)
                .body("userName", equalTo("Anne"));
    }
    
    @Test
    public void testRestForAllUserRoles() {
        login("Anne", "test");
        given()
                .contentType("application/json")
                .header("Authorization", "Bearer " + securityToken)
                .when()
                .get("/api/user/roles").then()
                .statusCode(200);
    }

    @Test
    public void testRestForAllBooks() {
        login("Peter", "test");
        given()
                .contentType("application/json")
                .header("Authorization", "Bearer " + securityToken)
    ...
    @Test
    public void testRestForBookById() {
        login("Peter", "test");
        given()
                .contentType("application/json")
                .header("Authorization", "Bearer " + securityToken)
